# Bài 1: Tự Động Hóa Tạo Video & Đăng YouTube Shorts Đa Cảnh Bằng AI Gemini, Edge-TTS, FFmpeg & Telegram Bot

![Sơ đồ tổng quan Workflow n8n YouTube Shorts Automation](https://raw.githubusercontent.com/eurusdevsec/AI-automation/main/docs/assets/buoi_5_banner.png)
...
## 🏛️ II. NỀN TẢNG KIẾN TRÚC & Ý NGHĨA CÔNG NGHỆ TRỌNG TÂM (TECH STACK RATIONALE)
...
5. Mở **Credentials** $\rightarrow$ **Create Credentials** $\rightarrow$ **OAuth client ID**:
   ...
   * Authorized redirect URIs: Nhập `http://localhost:5678/rest/oauth2-credential/callback`.
...
## ⚙️ IV. HƯỚNG DẪN CẤU HÌNH CHI TIẾT TỪNG NODE (16 NODES)
...
* **Credential**: Chọn Credential Gemini đã tạo ở Phần III.
* **JSON Output**: `true` (Bật công tắc này để ép Gemini trả về định dạng JSON thuần).
* **Prompt Content**:
```prompt
=Bạn là một nhà sáng tạo nội dung YouTube chuyên nghiệp và tuân thủ nghiêm ngặt Luật An ninh mạng Việt Nam.

QUY TẮC AN TOÀN NỘI DUNG (BẮT BUỘC KHÔNG ĐƯỢC VI PHẠM):
...
2. Trả về định dạng JSON chuẩn xác theo cấu trúc sau (không kèm markdown ```json):
{
  ...
  "description": "Mô tả video chuẩn SEO #Shorts",
  "tags": "trend, cong nghe, giai tri, vietnam, shorts",
  "scenes": [
    {
      "scene_number": 1,
...
```
* **Lý do dùng**: Ép Gemini trả về mảng `scenes` 5-6 phân cảnh có lời thoại tiếng Việt và câu lệnh vẽ ảnh tiếng Anh riêng biệt cho từng cảnh.
...
* **Credential**: Chọn YouTube OAuth2 Account đã tạo ở Phần III.
* **Resource**: `video`
* **Operation**: `upload`
* **Title**: `={{ $json.title }}`
...
4. Gán lại 3 Credentials đã tạo ở **Phần III**:
...
## 💡 VI. BẢN ĐỒ XỬ LÝ BẪY LỖI & THỰC CHIẾN (TROUBLESHOOTING)
...
## 🎯 VII. CHECKLIST HOÀN THÀNH BÀI HỌC
